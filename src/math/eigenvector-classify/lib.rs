...
pub fn keyed_acousticness<K: Eq + Hash>(ev: &[[f64; 3]], keys: &[K], mask: &[u32; 3]) -> f64
...
pub fn keyed_acoustic_basis<K: Eq + Hash>(keys: &[K], mask: &[u32; 3]) -> Basis<[f64; 3]>
{ filtered_keyed_basis(keys.iter().map(Some)).as_ref().on_axes(mask) }
...
fn filtered_keyed_basis<K, Ks>(keys: Ks) -> Basis<f64>
where
    K: Eq + Hash,
    Ks: IntoIterator<Item=Option<K>>,
    Ks::IntoIter: ExactSizeIterator,
{
    ...
    let mut data = by_key.values().map(|indices| {
        ...
        (0..n).map(move |i| indices.contains(&i) as i32 as f64)
    }).flat_map(|x| x).collect::<Vec<_>>();

    for ket in data.chunks_mut(n) {
