...
/// The full set of metadata kept available for the primitive cell.
pub type PrimMeta = HList3<
    meta::SiteElements,
    meta::SiteMasses,
    // FIXME: currently this is always None, and it is only carried around to limit the
    //        number of places in the code that need to be updated for proper support.
    //
    // Fixing this currently requires at least:
    //
    // * serializing the FracBonds to the phonopy directory (which feels redundant...)
    // * computing the new FracBonds of the supercell.
    // * The prior bullet requires DirWithDisps to have access to the bond length threshold.
    //
    // until this is fixed, potentials that require a bond graph cannot be used with phonopy.
    Option<meta::FracBonds>,
>;

/// The full set of metadata made available for the supercell.
pub type SuperMeta = PrimMeta;
...
pub struct DirWithDisps<P: AsPath> {
    ...
    pub(crate) super_meta: SuperMeta,
}
...
impl<P: AsPath> DirWithDisps<P> {
    ...
    {Ok({
        ...
        let bonds: Option<meta::FracBonds> = None; // FIXME: read saved bonds
        let meta = hlist![elements, masses, bonds];
    ...
    pub fn primitive_structure(&self) -> FailResult<(Coords, PrimMeta)>
    ...
    #[allow(unused)]
    pub fn superstructure(&self) -> (&Coords, SuperMeta)
...
impl<P: AsPath> DirWithDisps<P> {
    ...
    pub fn rsp2_style_displacements(&self) -> FailResult<Rsp2StyleDisplacements> {
        ...
        let our_super_meta = prim_meta.map(hlist![
            ...
            |x: Rc<[_]>| -> Rc<[_]> { sc.replicate(&x[..]).into() },
            |_| -> Option<meta::FracBonds> {
                // FIXME would have to recompute from scratch using `our_super_coords`,
                //       but this requires access to the bond length threshold.
                None
            }
        ]);
        ...
        Ok(Rsp2StyleDisplacements {
            ...
            super_meta: our_super_meta.sift(),
            sc,
            prim_displacements,
            perm_from_phonopy,
        })
...
impl<P: AsPath> DirWithForces<P> {
    ...
    #[allow(unused)]
    pub fn structure(&self) -> FailResult<(Coords, PrimMeta)>
    { read_input_structure_with_mass(self) }
...
fn masses_from_conf(conf: &Conf) -> Option<FailResult<meta::SiteMasses>>
{Some({
    conf.0.get("MASS")?
        .split_whitespace()
        .map(|s| s.parse().map(meta::Mass))
...
) -> FailResult<(Coords, PrimMeta)>
{
    ...
    let bonds = None; // FIXME
    let Poscar { coords, elements, .. } = Poscar::load(dir.join("POSCAR"))?;
    Ok((coords, hlist![elements.into(), masses, bonds]))
}
