...
fn compute(params: &Params, r_ij: V3, normal_i: V3, normal_j: V3) -> Output {
    ...
    debug_assert!(params.cutoff_begin >= 0.0);
    debug_assert!(params.cutoff_transition_dist >= 0.0);
    ...
    let (cutoff, d_cutoff_ddist) = match dist < params.cutoff_begin {
        true  => (1.0, 0.0),
        false => switch_poly3(params, dist)
    };

    // first get the attractive part of the potential
    let (v_attractive, d_attractive_dr) = attractive_part(dist_sq, r_ij);

    // then the repulsive
    let (v_rep, d_rep_dr, d_rep_dni, d_rep_dnj) = repulsive_part(dist_sq, r_ij, normal_i, normal_j);
...
fn switch_poly3(params: &Params, distance: f64) -> (f64, f64) {
...
fn attractive_part(dist_sq: f64, r: V3) -> (f64, V3) {
...
fn repulsive_part(dist_sq: f64, r: V3, normal_i: V3, normal_j: V3) -> (f64, V3, V3, V3) {
    debug_assert_eq!(dist_sq, r.sqnorm());
    debug_assert_close!(1.0, normal_i.sqnorm());
    debug_assert_close!(1.0, normal_j.sqnorm());
