...
use crate::interactions::{self, SiteI, BondI, IsInteracting};
...
fn _compute_rebo_bonds(
    ...
    let site_data = IndexVec::<SiteI, _>::from_raw({
        interactions.sites_maybe_parallel(use_rayon).map(|site_i| {
    ...
    let out = interactions.sites_maybe_parallel(use_rayon).map(|site_i| {
...
pub type Interactions = interactions::Interactions<InteractionsPotential>;
#[derive(Debug, Clone)]
pub struct InteractionsPotential {
    params: Params,
}

impl interactions::Potential for InteractionsPotential {
    ...
    {
        let params_ij = self.params.by_type[types.0][types.1];
        match distance {
    ...
    fn max_bonds() -> Option<usize> { Some(SITE_MAX_BONDS) }
}
