...
pub fn read<R: Read>(mut r: R) -> Result<Vec<Basis>> {
    let bytes = {
        let mut bytes = vec![];
...
pub fn parse(bytes: &[u8]) -> Result<Vec<Basis>> {
    match self::parse::npy(bytes).to_full_result() {
        Ok(x) => Ok(x),
        Err(e) => bail!("{:?}", e), // generic, not displayable...
    }
}
...
mod parse {
    ...
    named!{pub npy<Vec<Basis> >,
        do_parse!(
            ...
            //  kpoint, (atom * axis), bond, (real | imag)
